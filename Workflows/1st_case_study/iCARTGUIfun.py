...
def import_file(filename, sample_size=None, random_sampling=False, header=None, index_col=None, sep = ',', train_test_splitting=True, test_size=0.25, random_state=None):
    ...
    if random_sampling == False:
        dataset_df=iCART.Data_Preprocessing.Data_Preparation(filename=filename, header=header, index_col=index_col, sep = sep, train_test_splitting=train_test_splitting, 
        ...
        dataset_df=iCART.Data_Preprocessing.Data_Preparation(filename=dataset_sample, header=header, index_col=index_col, train_test_splitting=train_test_splitting, 
                                                             test_size=test_size, random_state=random_state)
...
class InteractiveDecisionTreesGUI():
    ...
    def interactive_plot(self, criterion, max_depth, max_features, max_leaf_nodes, min_impurity_decrease,min_samples_leaf, min_samples_split, random_state, splitter, nodes_coloring, edges_shape, 
        ...
        tree_base=tree.DecisionTreeClassifier(ccp_alpha=0, class_weight=None, criterion=criterion, max_depth=max_depth, max_features=max_features, 
                                              ...
                                              min_samples_split=min_samples_split, min_weight_fraction_leaf=0.0, presort='deprecated', random_state=random_state, 
                                              splitter=splitter)
        tree_base.fit(self.Data_dict['x'], self.Data_dict['y']) #it was: tree_base.fit(x,y) and iCART.TreeStructure(Tree=tree_base, features=features,  X=x, Y=outputs_complete_dataset, classes_labels=classes_labels, outputs_train=y)
        TreeStructureBase=iCART.TreeStructure(Tree=tree_base, features=self.Data_dict['features'],  X=self.Data_dict['x'], Y=self.Data_dict['outputs_complete_dataset'], 
                                              classes_dict=self.classes_dict, outputs_train=self.Data_dict['outputs_train'])
        iCART.Plot_Tree(TreeStructureBase, classes_dict=self.classes_dict, criterion=criterion, nodes_coloring=nodes_coloring, edges_shape=edges_shape, 
...
#         class_label_widget = widgets.Dropdown(options=list(set(self.Data_dict['y'])), value=list(set(self.Data_dict['y']))[0], description='Class Label',  disabled=False)
    ...
    def InterConAnalDTGUI(self):
        ...
        controls = widgets.HBox(inter_3.widget.children[:-1], layout = Layout(flex_flow='row wrap'))
        ...
        Feature_box=widgets.HBox([Variable_name_widget, Group_name_widget, Equation_widget, Equation_button, eq_out, Updt_Feat_List_button, featlist_out]) #, Refresh_plot_button, ref_out
        ...
        class modify_nodes():
            def __init__(self, Data_dict = self.Data_dict, classes_dict = self.classes_dict, Features_color_groups = self.Features_color_groups, tree_is_pruned = False, Pruned_tree = None): 
                ...
                if tree_is_pruned == False:
                    Tree_modf=tree.DecisionTreeClassifier(class_weight=None, criterion=inter_3.widget.kwargs['criterion'], max_depth=inter_3.widget.kwargs['max_depth'], 
                                                          ...
                                                          min_samples_split=inter_3.widget.kwargs['min_samples_split'], min_weight_fraction_leaf=0.0, presort='deprecated', 
                    ...
                    self.Tree_to_modify_str = iCART.TreeStructure(self.Tree_to_modify, Data_dict['features'], Data_dict['x'], Data_dict['outputs_complete_dataset'],
            ...
            def first_modification(self, Data_dict = self.Data_dict, classes_dict = self.classes_dict, Features_color_groups = self.Features_color_groups):
                change_thres_data=iCART.specify_feature_split_point(Tree=self.Tree_to_modify, features=Data_dict['features'], X=Data_dict['x'], Y=Data_dict['outputs_complete_dataset'],
            ...
            def modify(self, Data_dict = self.Data_dict, classes_dict = self.classes_dict, Features_color_groups = self.Features_color_groups):
                change_thres_data=iCART.specify_feature_split_point(Tree=self.Last_modified_tree, features=Data_dict['features'], X=Data_dict['x'], Y=Data_dict['outputs_complete_dataset'],
                                                                    outputs_train=Data_dict['y'], classes_dict=classes_dict, node_id=Node_id_widget.value, feature=Feature_to_Split_widget.value,
                                                                    new_threshold=Split_point_widget.value, print_rules=False, Best_first_Tree_Builder=True, criterion=inter_3.widget.kwargs['criterion'],
        ...
        Specify_feature_split_point_Box= widgets.HBox([Node_id_widget, Feature_to_Split_widget, Split_point_widget, max_leaf_nodes_left_widget, max_leaf_nodes_right_widget, 
        ...
        TreeStructure_orig=iCART.TreeStructure(Tree=tree_orig, features=self.Data_dict['features'],  X=self.Data_dict['x'], Y=self.Data_dict['outputs_complete_dataset'],
        ...
        class Pruning:
            def __init__(self, nd_id, Tree_to_prune, Data_dict = self.Data_dict, classes_dict = self.classes_dict, Features_color_groups = self.Features_color_groups, modified = False):
                ...
                if modified == True:
                    ...
                    TreeStructure_orig=iCART.TreeStructure(Tree=tree_orig, features=Data_dict['features'],  X=Data_dict['x'], Y=Data_dict['outputs_complete_dataset'],
            ...
            def first_pruning(self, Data_dict = self.Data_dict, classes_dict = self.classes_dict, Features_color_groups = self.Features_color_groups):
                treebase=iCART.ManualPruning(node_id=self.nd_id, TreeObject=self.Tree_to_prune, classes_dict=classes_dict, criterion=inter_3.widget.kwargs['criterion'], 
                                             nodes_coloring=inter_3.widget.kwargs['nodes_coloring'], edges_shape=inter_3.widget.kwargs['edges_shape'], 
                                             User_features_color_groups=None if inter_3.widget.kwargs['nodes_coloring'] != 'Features_color_groups' else Features_color_groups, 
            ...
            def pruning(self, nd_id, modified_tree=None, Data_dict = self.Data_dict, classes_dict = self.classes_dict, Features_color_groups = self.Features_color_groups):
                if tree_is_pruned_widget.value == False:  
                    treebase=iCART.ManualPruning(node_id=nd_id, TreeObject=self.Last_Pruned_tree, classes_dict=classes_dict, criterion=inter_3.widget.kwargs['criterion'], 
                                                 nodes_coloring=inter_3.widget.kwargs['nodes_coloring'], edges_shape=inter_3.widget.kwargs['edges_shape'], 
                                                 User_features_color_groups=None if inter_3.widget.kwargs['nodes_coloring'] != 'Features_color_groups' else Features_color_groups, 
                                                 Best_first_Tree_Builder=inter_3.widget.kwargs['Best_first_Tree_Builder'],
                                                 txt_size=inter_3.widget.kwargs['txt_size'], mrk_size=inter_3.widget.kwargs['mrk_size'], 
        ...
        Pruning_box= widgets.VBox([node_to_prune_widget, Prune_button, Tree_is_modified_widget, Refresh_widget, prune_out], box_style='info')
        ...
        def change_leaf_node_class(chg_cl):
            ...
            with change_leaf_node_class_out:
                if self.counter == 0 and self.counter_prn == 0:
                    global base
                    base = modify_nodes()
                    iCART.change_class(leaf_node_id_widget.value, base.Tree_to_modify_str, new_class_widget.value)
                    iCART.Plot_Tree(base.Tree_to_modify_str, self.classes_dict, criterion=inter_3.widget.kwargs['criterion'], Best_first_Tree_Builder=True, 
                                    nodes_coloring=inter_3.widget.kwargs['nodes_coloring'], edges_shape=inter_3.widget.kwargs['edges_shape'], 
                    ...
                    iCART.change_class(leaf_node_id_widget.value, base.Last_modified_tree, new_class_widget.value)
                    iCART.Plot_Tree(base.Last_modified_tree, self.classes_dict, criterion=inter_3.widget.kwargs['criterion'], Best_first_Tree_Builder=True,
                                    nodes_coloring=inter_3.widget.kwargs['nodes_coloring'], edges_shape=inter_3.widget.kwargs['edges_shape'], 
                                    User_features_color_groups=None if inter_3.widget.kwargs['nodes_coloring'] != 'Features_color_groups' else self.Features_color_groups,
                                    plot_width=inter_3.widget.kwargs['plot_width'], plot_height=inter_3.widget.kwargs['plot_height'], 
                                    mrk_size=inter_3.widget.kwargs['mrk_size'], txt_size=inter_3.widget.kwargs['txt_size'], 
                    ...
                    if Last_tree_interaction_widget.value == 'Last pruned':
                        iCART.change_class(leaf_node_id_widget.value, base_prn.Last_Pruned_tree, new_class_widget.value)
                        iCART.Plot_Tree(base_prn.Last_Pruned_tree, self.classes_dict, criterion=inter_3.widget.kwargs['criterion'], Best_first_Tree_Builder=True,
                                        nodes_coloring=inter_3.widget.kwargs['nodes_coloring'], edges_shape=inter_3.widget.kwargs['edges_shape'], 
                                        User_features_color_groups=None if inter_3.widget.kwargs['nodes_coloring'] != 'Features_color_groups' else self.Features_color_groups,
                                        plot_width=inter_3.widget.kwargs['plot_width'], plot_height=inter_3.widget.kwargs['plot_height'],
                                        mrk_size=inter_3.widget.kwargs['mrk_size'], txt_size=inter_3.widget.kwargs['txt_size'], 
        ...
        Changing_class_box = widgets.VBox([Changing_class_inputs_box, change_leaf_node_class_out], box_style='info')
            ...
            with accuracy_out:
                if Tree_state_widget.value == 'No expert tree interactions':
                    ...
                    if self.Train_test_splitting.value == True:
                        ...
                        predicted_iDT_train=iCART.classify(base.Last_modified_tree, self.Data_dict['x'])
                        predicted_iDT_train.classify()
                        #Make predictions on test set:      
                        predicted_iDT_test=iCART.classify(base.Last_modified_tree, self.Data_dict['z'])
                        ...
                        predicted_iDT=iCART.classify(base.Last_modified_tree, self.Data_dict['x'])
                    ...
                    if self.Train_test_splitting.value == True:
                        #Make predictions on train set:
                        predicted_iDT_train=iCART.classify(base_prn.Last_Pruned_tree, self.Data_dict['x'])
                        predicted_iDT_train.classify()
                        #Make predictions on test set:      
                        predicted_iDT_test=iCART.classify(base_prn.Last_Pruned_tree, self.Data_dict['z'])
                        predicted_iDT_test.classify()
                        Accuracies = {'Train': [accuracy_score(self.Data_dict['y'], predicted_iDT_train.predicted_classes)],
                                      'Test' : [accuracy_score(self.Data_dict['w'], predicted_iDT_test.predicted_classes)]}
                        self.Accuracies = pd.DataFrame(Accuracies, index = ['Classification Accuracy'])
                    elif self.Train_test_splitting.value == False:
                        predicted_iDT=iCART.classify(base_prn.Last_Pruned_tree, self.Data_dict['x'])
                        predicted_iDT.classify()
                        Accuracies = {'Dataset': [accuracy_score(self.Data_dict['y'], predicted_iDT.predicted_classes)]}
                        self.Accuracies = pd.DataFrame(Accuracies, index = ['Classification Accuracy'])
            ...
            with Output_tree_out:
                if Tree_state_widget.value == 'No expert tree interactions':
                    tree_orig=tree.DecisionTreeClassifier(criterion=inter_3.widget.kwargs['criterion'], splitter=inter_3.widget.kwargs['splitter'], max_depth=inter_3.widget.kwargs['max_depth'],
                                                          ...
                                                          min_impurity_split=None, class_weight=None, presort='deprecated',ccp_alpha=0.0) 
                    tree_orig.fit(self.Data_dict['x'],self.Data_dict['y'])
                    TreeStructureBase_orig=iCART.TreeStructure(Tree=tree_orig, features=self.Data_dict['features'],  X=self.Data_dict['x'], Y=self.Data_dict['outputs_complete_dataset'], 
                                                               classes_dict=self.classes_dict, outputs_train=self.Data_dict['outputs_train'])
                    if self.Train_test_splitting.value == True:
